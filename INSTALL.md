...
从 harness 仓库读取 `.claude/skills/agent-team/orchestrator.md` 的完整内容并创建到目标项目。

> **Agent 注意**：以下各小节的文件内容，通过 WebFetch 或 Read 从 harness 仓库对应的
> `.claude/skills/agent-team/` 目录下的同名文件中获取，内容完全复制，不做任何修改。

### 7.2 创建 `.claude/skills/agent-team/feature-developer.md`

从 harness 仓库读取 `.claude/skills/agent-team/feature-developer.md` 并创建到目标项目。

### 7.3 创建 `.claude/skills/agent-team/maintenance-developer.md`

从 harness 仓库读取 `.claude/skills/agent-team/maintenance-developer.md` 并创建到目标项目。

### 7.4 创建 `.claude/skills/agent-team/test-engineer.md`

从 harness 仓库读取 `.claude/skills/agent-team/test-engineer.md` 并创建到目标项目。

### 7.5 创建 `.claude/skills/agent-team/code-reviewer.md`

从 harness 仓库读取 `.claude/skills/agent-team/code-reviewer.md` 并创建到目标项目。

### 7.6 创建 `.claude/skills/agent-team/security-auditor.md`

从 harness 仓库读取 `.claude/skills/agent-team/security-auditor.md` 并创建到目标项目。

### 7.7 创建 `.claude/skills/agent-team/documentation-keeper.md`

从 harness 仓库读取 `.claude/skills/agent-team/documentation-keeper.md` 并创建到目标项目。

### 7.8 创建 `.claude/skills/agent-team/devops-engineer.md`

从 harness 仓库读取 `.claude/skills/agent-team/devops-engineer.md` 并创建到目标项目。

### 7.9 创建 `.claude/skills/agent-team/monitor.md`

从 harness 仓库读取 `.claude/skills/agent-team/monitor.md` 并创建到目标项目。
