...
harness 是纯内容仓库。**禁止**提交任何可执行代码文件：

- 禁止扩展名：`.js` `.ts` `.jsx` `.tsx` `.py` `.sh` `.rb` `.go` `.java` `.rs` `.c` `.cpp`
- 所有安装逻辑通过 Markdown 指令描述，由 Agent 解释执行

### 约束 2：命令文件不依赖外部 URL

`.claude/commands/` 和 `INSTALL.md` 中的命令必须可离线执行：

- 不得包含需要 Agent 运行时访问的 `http://` 或 `https://` URL
- 用户指引类的 URL（如 GitHub Issues 页面模板）应通过变量替换呈现，不硬编码域名
